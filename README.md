...
> **Язык документации:** [🇷🇺 Русский](./README.md) | [🇬🇧 English](./README.en.md)
...
- [Запуск воркера (CLI)](#запуск-воркера-cli)
- [Развёртывание в Docker](#развёртывание-в-docker)
- [HTTP API](#http-api)
- [Отчёты](#отчёты)
- [Файлы состояния (checkpoint)](#файлы-состояния-checkpoint)
- [Интеграция с AI](#интеграция-с-ai)
- [Лимиты и параллелизм](#лимиты-и-параллелизм)
...
3. 🏷️ **Метки строк** — каждая строка получает префикс с относительным путём, например: `[domen.ru/access.log] 1.2.3.4 - - [...]`.
...
| **Доступ к логам** | — | Права на чтение `/var/log/nginx` |
| **API-ключ AI** | — | `OPEN_AI_KEY` — OpenAI, AITunnel, локальный прокси и т.д. |
...
<details>
<summary>📌 Пример для AITunnel</summary>
...
### 🕰️ Устаревшие (обратная совместимость)
...
> ⚠️ Без постоянного тома checkpoint и отчёты теряются при пересоздании контейнера.

### ⚙️ Переопределения в compose
...
> ⚠️ **Аутентификация не реализована** — в продакшене ограничивайте доступ файрволом, reverse proxy или VPN.
...
| # | Правило |
|---|---------|
| 1 | ⚠️ **API без аутентификации** — не публикуйте порт `3010` в интернет без защиты. |
...
| 5 | 🛡️ **Имена отчётов** проверяются на path traversal (`..` запрещён). |
